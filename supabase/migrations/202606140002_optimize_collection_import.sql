...
create index if not exists card_variants_collection_lookup_idx
...
begin
  ...
  select
    ...
    trim(coalesce(source.collector_number, '')) as collector_number_value,
  ...
  left join lateral (
    select
      ...
      (array_agg(variant.id order by variant.id))[1] as variant_id
    from public.card_variants variant
    join public.card_search search_card
      on search_card.oracle_id = variant.oracle_id
    where variant.lang = 'en'
      and lower(variant.set_code) = source.set_code_value
      and lower(variant.collector_number) = lower(source.collector_number_value)
      and lower(variant.name) = lower(source.card_name)
  ) exact_match
  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'error', case
          ...
          when match_count > 1
            then 'Card matched more than one record. Include set and collector number.'
        end
      ) as error_row
    from collection_import_resolved
    where card_name = ''
    ...
    select
      coalesce(nullif(split_part(source_rows, ',', 1), '')::integer, 0) as sort_row,
      jsonb_build_object(
